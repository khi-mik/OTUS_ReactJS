...
export const power2: AlgorithmicOperationType = (first: number): number =>
  first * first;

export const factorial: AlgorithmicOperationType = (first: number): number =>
  first ? first * factorial(first - 1) : 1;

export const sin: AlgorithmicOperationType = (first: number): number =>
  Math.sin(first);

export const cos: AlgorithmicOperationType = (first: number): number =>
  Math.cos(first);

export const tan: AlgorithmicOperationType = (first: number): number =>
  Math.tan(first);

export const fibonacci: AlgorithmicOperationType = (first: number): number =>
  first !== 0 && first !== 1
    ? fibonacci(first - 1) + fibonacci(first - 2)
    : first;
